...
import whoGrowthStandards from '@/data/who_growth_standards_0-5.json';
...
// Function to determine stunting status based on length/height-for-age Z-score
export function determineStuntingStatus(zScore: number | null): string {
  if (zScore === null) return 'Tidak Diketahui';
...
// Function to determine weight status based on weight-for-age Z-score
export function determineWeightStatus(zScore: number | null): string {
  if (zScore === null) return 'Tidak Diketahui';
  
  if (zScore < -3) return 'Sangat Kurus';
  if (zScore < -2) return 'Kurus';
  if (zScore < 2) return 'Berat Normal';
  return 'Berat Lebih';
}
